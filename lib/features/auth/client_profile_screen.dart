...
class _ClientProfileScreenState extends ConsumerState<ClientProfileScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final assignedPropertiesCount = propertiesAsync.when(
      data: (items) => items.length,
      loading: () => session.assignedPropertyIds.length,
      error: (_, __) => session.assignedPropertyIds.length,
    );
    final currentFirstName = profile?.username.isNotEmpty == true
        ? profile!.username
        : (session.userName ?? l10n.clientLoginTab);
        ...
        children: [
          ...
          Card(
            child: ListTile(
              ...
              title: Text(l10n.assignedProperties),
              subtitle: Text(
                l10n.assignedPropertiesCount(assignedPropertiesCount),
              ),
